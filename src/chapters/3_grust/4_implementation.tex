...
As explained in \Cref{sec:intro:gap}, \ampere is adopting asynchronous \Rust to realize the
...
\begin{itemize}
    ...
    \item \texttt{grust-std}: This crate acts as the standard library for \GRust, offering a
          ...
          execution model.\footnote{The correctness of time and memory-dependent components, such as
              ...
              \Cref{sec:semantics:grsync:react_typing}.}
\end{itemize}
...
implements the component's semantics, formalized in \Cref{sec:semantics:grsync:opsem}.
...
\begin{enumerate}
    ...
    \item \textbf{Type Analysis:} The compiler performs type checking according to the type system
          ...
          \Cref{sec:semantics:grsync:react_typing}. This system is crucial for guaranteeing that a
          component's behavior remains consistent when used within an asynchronous, event-driven
          \GRFRP service.
          ...
          While this verification is a key part of the language's formal semantics, its
          implementation in the prototype compiler is not yet complete. This check would logically
          follow the main type analysis pass, enriching the symbol table with reactive type
          information before subsequent stages.
...
\begin{description}
    \item[Context Struct]
          The \emph{Context Struct} holds the memories needed for future computations.
          These values, such as the last value of a signal, correspond to the state
          defined in the operational semantics (\Cref{sec:semantics:grfrp:opsem}).
          Each stored value is paired with a boolean flag to indicate whether it has been
          updated in the current execution step.
...
\begin{enumerate}
    ...
    \item \textbf{Type Analysis:} The compiler performs type checking according to the type system
          ...
          is formally detailed in \Cref{sec:semantics:grfrp:react_typing}. This system is essential
          for ensuring that service behavior remains predictable and compositional in an
          event-driven, push-based execution model.
          ...
          As with its \GRSync counterpart, the implementation of this reactive type checking is
          planned but not yet complete in the prototype compiler. It would logically follow the
          standard type analysis pass, further annotating the symbol table before code generation.
...
Kind~2~\cite{DBLP:conf/cav/ChampionMST16}, which uses $k$-induction to verify properties of stateful
