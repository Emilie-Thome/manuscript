...
\Cref{sec:intro:gap} of \Cref{cha:intro}. This pull-based, periodic approach is ill-suited for efficiently modeling the
...
The current landscape presents a clear dichotomy. On one side, well-established, standard tools
like \Simulink and \Scade offer mature, high-level modeling abstractions but are tied to an
inefficient, pull-based execution model. On the other side, emerging \Rust-based stacks embrace the
correct event-driven architecture but fail to provide the high-level, formally-grounded modeling
abstractions needed for complex control systems.

Neither of these worlds successfully marries the rigor of synchronous, model-based design with the
safety and asynchronous capabilities of the modern \Rust-based \sdv platform. This unfilled gap is
precisely where \GRust is positioned. It is designed to provide the formal, declarative modeling of
a synchronous language while directly generating safe asynchronous \Rust code\footnote{However, the
  generated code relies on libraries like \texttt{tokio}, which use internal \texttt{unsafe} code.},
bridging the gap that current industrial and emerging tools leave open.
...
As introduced in \Cref{sec:intro:dsl:dataflow} of \Cref{cha:intro}, a dataflow model describes a system as a network of
...
\Cref{tab:sota:synchronous:lustre:exec} shows an execution trace of the program (a chronogram). The
node computes two outputs, \texttt{CLK} and \texttt{O}, from the input \texttt{I}. The equation
...
offering a more structured and accessible way to model state-based behavior. This suggests a trend
toward providing abstractions that are easier to use than the underlying formal mechanisms.
...
first-class citizens in the language.\footnote{Although modern versions of \Lustre treat clocks as
  first-class citizens for greater flexibility~\cite{DBLP:conf/emsoft/ColacoP03}, the concept was
  originally introduced as a system of dependent types~\cite{DBLP:conf/icfp/CaspiP96}.} This
calculus allows a programmer to declare clock variables of type \texttt{event}, extract the clock
from a signal (\eg \texttt{e = when s}), and define explicit equations over these clocks. For
instance, the expression \verb|o ^= a ^+ b| specifies that the clock of \texttt{o} is the union of
the clocks of \texttt{a} and \texttt{b}. By making timing relationships an integral part of the
program specification, \Signal provides a powerful formalism for describing complex, multi-rate,
...
\begin{multicols}{2}[The classical \texttt{ABRO} module below illustrates the imperative style of
    \Esterel.]
  \begin{lstlisting}[basicstyle=\ttfamily\footnotesize\color{BlackText}]
  module ABRO:
    input A, B, R;
...
\textsc{HipHop.js}, which serves to integrate a single, time-consuming asynchronous operation into
a synchronous execution thread. The approach of \textsc{HipHop.js} is conceptually similar to
futures in \Lustre~\cite{DBLP:conf/emsoft/CohenGP12}, where tasks are launched and their results
...
\GRFRP, the programmer declares in a functional style to define the relationships between
...
\GRust thus brings the rigor of synchronous programming and the augmented safety of the \Rust
language to the implementation of individual components within a larger, ROS-like asynchronous
architecture, abstracting away the need for manual locking and imperative propagation logic.
...
enforcing global determinism like \textsc{Lingua Franca}, or leaving it to the programmer like ROS,
...
(\Cref{sec:semantics:grsync:react_typing,sec:semantics:grfrp:react_typing} in \Cref{cha:semantics}). This
...
simple reactive computation. The syntaxes used here are the formal syntaxes presented in the papers,
they reflect the core principles of each approach.
...
every single input, inducing more computations than necessary, thus wasting resources. To address
this, the extension \textsc{Async Rattus}~\cite{DBLP:journals/pacmpl/BahrM23} adapts the modal model
with dynamic, local clocks, enabling efficient asynchronous computation.
...
Practical libraries like \ReactiveX are efficient but lack a clear formal semantics, a weakness that
becomes apparent in stateful feedback loops. Without formal safeguards, it is possible to create an
\emph{unguarded feedback loop}, where the output of a stream is fed back into its input within the
same computational step.
%
The following \ReactiveX example (in RxPy) illustrates this problem. A \texttt{feedback\_subject} is
subscribed to a transformed version of itself, creating an instantaneous cycle. Each value emitted
by the subject is incremented and immediately re-emitted, causing an infinite recursion.

\begin{lstlisting}[basicstyle=\ttfamily\footnotesize\color{BlackText}]
import reactivex as rx
from rx import operators as ops

feedback_subject = rx.subject.BehaviorSubject(1)
feedback_subject.pipe(ops.map(lambda x: x + 1)).subscribe(feedback_subject)
feedback_subject.subscribe(print)
\end{lstlisting}

\GRust addresses this by formalizing the efficient, push-based execution model. The reactive type
system of the language (\Cref{sec:semantics:grsync:react_typing,sec:semantics:grfrp:react_typing} in
\Cref{cha:semantics}) guarantees that for any well-typed service, this event-driven execution is
semantically equivalent to a more easily understood periodic one. This approach provides the
efficiency of practical \frp implementations with the correctness guarantees of formal synchronous
models.
...
their objectives differ fundamentally. \Heptagon/BZR focuses on \emph{synthesis}, that is the
automatic generation of an implementation from a high-level contract. In contrast, \GRust, through
\GReusot, focuses on the \emph{deductive verification} of a manually written implementation. This
aligns with a more traditional engineering workflow where the developer writes the control logic and
a verifier confirms its correctness.
...
This is similar to the compilation of \FramaC contracts by \LustreC. However, \GRust's choice of
\Rust as a compilation target eliminates the generated assertions required to prove memory safety in
\Ci with \FramaC. The ownership and borrow-checking system of the \Rust compiler provides these
guarantees by construction, for free.
...
pragmatic position within this landscape, defined by a series of design choices.
...
and flexible engineering workflow where the developer has control over the implementation logic, and
the formal method is used to verify its correctness.
...
The difference is that safe \Rust eliminates the need to prove memory safety, as it should be done
with \Ci programs.
...
Traditional tools like \Simulink and \Scade offer mature modeling environments but are tied to a
periodic execution model, which was shown to be inefficient in the context of \sdv. Synchronous
languages such as \Lustre and \Esterel provide powerful formalisms for correctness, with derivatives
like \Prelude and \textsc{HipHop.js} adapting these principles to multi-periodic or asynchronous
environments. \frp variants solve theoretical issues of resource management, while Modal \frp
introduces advanced type systems to guarantee safety. Conversely, frameworks like ROS and \ReactiveX
embrace asynchrony but delegate the burden of ensuring correctness to the developer.
