...
First, the formal verification of the language semantics was conducted by hand. Although this
...
Second, the verification capabilities of \GReusot face a key limitation in the compositional
...
suggest (\Cref{sec:grust:benchmarks:grust_vs_rust} in \Cref{cha:grust}), this shift can lead to significant productivity
...
(\Cref{sec:grust:benchmarks:grust_vs_rust} in \Cref{cha:grust}). The prototype compiler was developed with a focus on
...
The first priority is to enhance the practical implementation of the \GRust compiler, with a focus
on improving its diagnostic capabilities.

\paragraph{Errors from the Reactive Type System}
A primary objective is to improve the error reporting for the \emph{reactive type system} (detailed
in \Cref{sec:semantics:grsync:react_typing,sec:semantics:grfrp:react_typing}). This type system is
critical for guaranteeing the soundness of the execution model of \GRust, as it formally proves that
a component is \emph{reactive}---resilient to oversampling and safe for event-driven execution. Its
central purpose is to statically distinguish between \emph{continuous} flows (those dependent on
execution time, like \timeop) and \emph{discrete} flows (those only driven by input changes).
%
Although the type system is fully implemented, the diagnostic messages produced by the prototype
compiler are not sufficiently informative to help developers resolve type violations. Future work
must focus on generating clear, actionable diagnostics that provide context for an error and suggest
potential solutions, possibly based on common patterns. This enhancement is a crucial step toward
delivering the predictability guarantees promised by the \GRust language.
